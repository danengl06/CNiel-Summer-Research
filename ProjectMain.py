...
# ADDED CLUSTERING STEP ----------------------------------------------------------------
    ...
    for iteration in range(max_iter):
        ...
        elif method_idx == 1:  # coefficient weighted average
            for sample_id, sample in enumerate(windows):
                cluster_id = labels[sample_id]
                shift = shifts[sample_id]
                x_shifted = sample[shift:shift+centroid_length]
                # Note: nnls uses the *current iteration's* centroid estimates
        ...
        elif method_idx == 3:  # normalized row rank-1 SVD
            X_shifted = np.zeros((n_samples, centroid_length))
            for sample_id, sample in enumerate(windows):
                shift = shifts[sample_id]
                x_shifted = sample[shift:shift+centroid_length]
...
for run_index in range(n_runs):
    ...
    for noise_factor in range(n_noise):
        ...
        for method_idx, method_name in enumerate(method_names):
           ...
           # Prints min, mean, max, SSE, and NNSSE
...
# 
# Average Prints
print("\n=== Average Performance Across All Noise Levels and Runs ===")

# Average over noise_factor and run_index
avg_stats = np.mean(perf_stats, axis=(2, 3))

for method_idx, method_name in enumerate(method_names):
    avg_min, avg_mean, avg_max, avg_sse, avg_nnsse = avg_stats[method_idx]
    print(f"{method_name:<12} | Avg Min: {avg_min:.3f} | Avg Mean: {avg_mean:.3f} | "
          f"Avg Max: {avg_max:.3f} | Avg SSE: {avg_sse:.3f} | Avg NNSSE: {avg_nnsse:.3f}")
